...
func _physics_process(delta: float) -> void:
	...
	var maneuverability = clamp((hp_left + hp_right) / MAX_HP * 1.5, 0.0, 1.0)
	var rot = 10 * PI / 180 * maneuverability

	if direction != last:
		body.rotate(last, rot)
		body.rotate(-direction, rot)
		last = direction

	if (node.position.x + (-direction.z * SPEED * maneuverability) > 60
		or node.position.x + (-direction.z * SPEED * maneuverability) < -60):
		direction.z = 0
	if (node.position.y + (direction.x * SPEED * maneuverability) > 30
		or node.position.y + (direction.x * SPEED * maneuverability) < -30):
		direction.x = 0

	node.translate(Vector3(direction.z, direction.x, 0) * SPEED * maneuverability)
